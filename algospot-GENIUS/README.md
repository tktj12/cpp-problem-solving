...
* 풀이
  ...
  * $prev$ : song 전에 재생된 곡, $L(i)$ : 노래 i의 재생시간.
  * $start(time, song) = $
...
* 풀이
  ...
  * 그러므로 $C_time$을 다음과 같이 정의하면 확률 $T$를 이용해 $C_{time+1} = W*C_{time}$를 만족하는 가중치 행렬 $W$ 또한 계산할 수 있다.

$$begin{vmatrix} start(time-3, 0) \\ 
vdots \\ 
start(time-3, n-1) \\ 
start(time-2, 0) \\ 
vdots \\ 
