...
* 풀이
  * $HasPath(begin, end)$ : 정렬된 간선의 배열에 $begin$번 간선 부터 $end$번 간선까지 사용해서 $0$번과 $N-1$번 정점을 
  잇는 경로를 만들 수 있는지
  * $begin$, $end$ 값을 늘려가며 답을 찾는다. 이때 $HasPath$함수가 $false$를 반환했다면 $begin$을 늘려봤자 $true$가 나올 리가 
  없기 때문에 $end$를 늘려야 하고, $true$를 반환했다면 $begin$을 늘려서 범위를 더 좁힌다.
