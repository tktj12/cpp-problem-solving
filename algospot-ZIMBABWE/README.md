...
# Solution
* m으로 나눠지는지 확인하려면 일일이 숫자를 만들어봐야 하는 거 아닌가?
...
* 풀이
  ...
  * $rest$ : 바로 전 까지 만든 수를 m으로 나눈 나머지. 나머지연산의 성질을 이용하기 위한 변수.

* 시간 복잡도 :
  * 부분문제의 개수가 $taken* is_less* rest$개 즉,  $2^n* 2* m$개 이고 부분문제별로
$n$개의 for문을 돎으로
  * $O(nm2^n)$
<br></br>
...
* 따라서 $taken$과 $is_less$, $rest$(나머지)가 같다면 이후에 가능한 수의 개수도 같다는 게 성립한다.